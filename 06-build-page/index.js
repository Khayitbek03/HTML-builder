...
async function copyAssets(sourceFolder, projectDistFolder) {
  try {
    ...
    files.forEach((file) => {
      if (file.isFile()) {
        fs.copyFile(
          path.join(sourceFolder, file.name),
          path.join(projectDistFolder, file.name),
          (err) => {
            if (err) throw err;
          },
        );
...
async function replacementHTML(file) {
  try {
    await fsPromises
      .readFile(path.join(__dirname, 'components', file.name))
...
async function dataHTML() {
  try {
    ...
    let tempHTML = await fsPromises.readFile(
    ...
    for (const file of files) {
      ...
      const resHTML = fs.createWriteStream(
